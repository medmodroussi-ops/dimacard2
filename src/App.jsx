...
const productsData = {
  'etudiant': {
    ...
    features: ['Carte PVC Noir Mat Standard', 'Profil digital de base', 'Lien vers CV ou Portfolio'],
  },
  'starter': {
    id: 'starter',
    name: 'Pack Starter',
  ...
  'pro': {
    ...
    features: ['Carte PVC Premium Or', 'Profil 100% Modifiable', 'Accès plateforme & Statistiques'],
  },
  'entreprise': {
    id: 'entreprise',
  ...
  useEffect(() => {
    ...
    s.textContent = `
      ...
      *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
      ...
      :root {
        /* NOUVELLES COULEURS : OR / AMBRE & VIOLET */
        --brand-primary: #F5A623;      /* Ambre/Or */
        --brand-primary-light: #FDE047;
        --brand-primary-dark: #D97706;
        --brand-secondary: #8B5CF6;    /* Violet */
        --brand-accent: #06B6D4;       /* Cyan profond pour les contrastes */
        
        --bg-main: #0B0F19;            /* Fond principal très sombre */
        --bg-surface: #111827;         /* Surfaces / Sections */
        --bg-card: #1F2937;            /* Cartes / Inputs */
        
        --text-main: #F9FAFB;          /* Texte clair */
        --text-muted: #9CA3AF;         /* Texte grisé */
      ...
      @keyframes nfcPulse { 0% { opacity: 0.2; transform: scale(0.9); } 50% { opacity: 1; transform: scale(1.05); filter: drop-shadow(0 0 12px var(--brand-primary)); } 100% { opacity: 0.2; transform: scale(0.9); } }
      ...
      .brand-logo-text { font-family: var(--font-logo); color: var(--brand-primary); text-shadow: -2px 3px 0px var(--brand-secondary); transform: skewX(-6deg); line-height: 1; letter-spacing: 2px; }
      ...
      @media (max-width: 768px) {
        ...
        .contact-grid, .footer-grid { grid-template-columns: 1fr !important; gap: 40px !important; text-align: center; }
...
function DimacardPhysical() {
  const bg = '#111827'; // Dark card base
  const border = '1px solid rgba(255, 255, 255, 0.1)'; 
  const logoCol = 'var(--brand-primary)'; 
  const textCol = '#FFFFFF'; 
  const glitchShadow = '3px -3px 0px var(--brand-accent), -3px 3px 0px var(--brand-secondary)';

  return (
    <div style={{ position: "absolute", inset: 0, borderRadius: 24, background: bg, border: border, boxShadow: "20px 40px 60px rgba(0,0,0,0.6), inset 0 2px 20px rgba(139, 92, 246, 0.15)", padding: "24px 8%", display: "flex", flexDirection: "column", justifyContent: "space-between", overflow: "hidden", transformStyle: "preserve-3d" }}>
      {/* Background texture for premium feel */}
      <div style={{ position: "absolute", inset: 0, background: "radial-gradient(circle at top right, rgba(245, 166, 35, 0.1), transparent 50%)" }} />
      ...
      <div className="card-sheen-anim" style={{ position: "absolute", top: 0, left: "-100%", width: "50%", height: "100%", background: "linear-gradient(105deg,transparent 30%,rgba(255,255,255,0.15) 50%,transparent 70%)", pointerEvents: "none", transform: "translateZ(1px)" }}/>
      ...
      <div style={{ position: "absolute", right: "8%", bottom: "20%", display: "flex", alignItems: "center", gap: 8, transform: "rotate(-45deg) translateZ(20px)" }}>
        <div className="nfc-wave-1" style={{ width: 10, height: 10, borderRadius: "50%", background: logoCol, boxShadow: `0 0 12px ${logoCol}` }} />
      ...
      <div style={{ position: "relative", zIndex: 2, transform: "translateZ(15px)", marginBottom: "2%" }}>
        <span style={{ fontFamily: "var(--font-body)", fontSize: "clamp(8px, 2.5vw, 10px)", color: textCol, letterSpacing: "2px", textTransform: "uppercase", fontWeight: 700, opacity: 0.8 }}>PREMIUM EDITION</span>
      </div>
    </div>
  );
}
...
function Navbar({ currentPage, setCurrentPage }) {
  ...
  return (
    <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 50, transition: "all 0.5s", padding: scrolled ? "12px 0" : "20px 0", background: scrolled ? "rgba(11, 15, 25, 0.85)" : "transparent", backdropFilter: scrolled ? "blur(20px)" : "none", borderBottom: scrolled ? "1px solid rgba(255,255,255,0.05)" : "none", boxShadow: scrolled ? "0 4px 30px rgba(0,0,0,0.5)" : "none" }}>
      <div style={{ maxWidth: 1280, margin: "0 auto", padding: "0 24px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 32 }}>
          <div className="nav-links hide-mobile">
            <button onClick={() => setCurrentPage('home')} style={{ background: "none", border: "none", cursor: "pointer", fontFamily: "var(--font-body)", fontSize: 14, fontWeight: 600, color: currentPage === 'home' ? "var(--brand-primary)" : "var(--text-muted)", transition: "color 0.2s" }}>
              Accueil
            </button>
            <button onClick={() => setCurrentPage('product_pro')} style={{ background: "none", border: "none", cursor: "pointer", fontFamily: "var(--font-body)", fontSize: 14, fontWeight: 600, color: currentPage.startsWith('product') ? "var(--brand-primary)" : "var(--text-muted)", transition: "color 0.2s" }}>
              Boutique
            </button>
          </div>
          <a href="#contact" style={{ padding: "10px 24px", borderRadius: 100, background: "var(--brand-primary)", color: "#000", fontFamily: "var(--font-body)", fontWeight: 800, fontSize: 13, textDecoration: "none", boxShadow: "0 4px 20px rgba(245, 166, 35, 0.25)", transition: "all 0.3s" }} onMouseEnter={e => e.currentTarget.style.transform="translateY(-2px)"} onMouseLeave={e => e.currentTarget.style.transform="translateY(0)"}>Contact</a>
        </div>
      </div>
    </nav>
  );
...
function Hero({ setCurrentPage }) {
  ...
  return (
    <section className="section-padding" style={{ position: "relative", minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden", perspective: "1000px", background: "var(--bg-main)" }}>
      <Particles count={25} color="var(--brand-primary)" />
      ...
      <div className="hide-mobile" style={{ position: "absolute", inset: 0, pointerEvents: "none", transformStyle: "preserve-3d" }}>
        ...
        <div style={{ position: "absolute", top: "15%", right: "5%", width: 400, height: 400, borderRadius: "50%", background: "radial-gradient(ellipse,rgba(139, 92, 246, 0.05) 0%,transparent 70%)", transform: `translateY(${mousePos.y * -40}px) translateX(${mousePos.x * -40}px) translateZ(-100px)`, transition: "transform 0.4s ease-out" }}/>
      </div>
        ...
        <div className="hero-grid" style={{ display: "grid", gap: 64, alignItems: "center" }}>
          <div className="hero-content mobile-no-3d" style={{ display: "flex", flexDirection: "column", transform: `translateZ(30px) rotateY(${mousePos.x * 5}deg) rotateX(${mousePos.y * -5}deg)`, transition: "transform 0.3s ease-out", transformStyle: "preserve-3d" }}>
            <div className="fade-up" style={{ animationDelay: "0.1s", display: "inline-flex", alignItems: "center", gap: 8, padding: "6px 16px", borderRadius: 100, background: "rgba(245, 166, 35, 0.1)", border: "1px solid rgba(245, 166, 35, 0.2)", marginBottom: 24 }}>
              <div style={{ width: 8, height: 8, borderRadius: "50%", background: "var(--brand-primary)", boxShadow: "0 0 10px var(--brand-primary)" }}/>
              <span style={{ fontFamily: "var(--font-body)", fontSize: 12, fontWeight: 700, color: "var(--brand-primary-light)", letterSpacing: "0.15em", textTransform: "uppercase" }}>Sans contact. Sans limite.</span>
            </div>
            <h1 className="hero-title fade-up" style={{ animationDelay: "0.2s", fontFamily: "var(--font-display)", fontSize: "clamp(42px,7vw,76px)", fontWeight: 900, color: "var(--text-main)", lineHeight: 1.05, letterSpacing: "-1px", marginBottom: 24 }}>
              L'Avenir des<br />Cartes de<br /><span style={{ color: "var(--brand-primary)", textShadow: "0 0 40px rgba(245,166,35,0.3)" }}>Visite.</span>
            </h1>
            ...
            <div className="hero-buttons fade-up" style={{ animationDelay: "0.5s", display: "flex", gap: 16, marginBottom: 44 }}>
              <button onClick={() => setCurrentPage('product_pro')} style={{ display: "inline-flex", alignItems: "center", gap: 10, padding: "16px 32px", borderRadius: 16, background: "var(--brand-primary)", color: "#000", fontFamily: "var(--font-body)", fontWeight: 800, fontSize: 15, border: "none", cursor: "pointer", boxShadow: "0 10px 30px rgba(245, 166, 35, 0.3)", transition: "all 0.3s" }} onMouseEnter={e => e.currentTarget.style.transform="translateY(-2px)"} onMouseLeave={e => e.currentTarget.style.transform="translateY(0)"}>
                Obtenir ma carte
              </button>
              <a href="#showcase" style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "16px 32px", borderRadius: 16, background: "rgba(255,255,255,0.05)", border: "1px solid var(--border-light)", color: "var(--text-main)", fontFamily: "var(--font-body)", fontWeight: 700, fontSize: 15, textDecoration: "none", transition: "all 0.3s" }} onMouseEnter={e => {e.currentTarget.style.background="rgba(255,255,255,0.1)"; e.currentTarget.style.borderColor="rgba(255,255,255,0.2)"}} onMouseLeave={e => {e.currentTarget.style.background="rgba(255,255,255,0.05)"; e.currentTarget.style.borderColor="var(--border-light)"}}>Comment ça marche</a>
            </div>
          </div>
          ...
          <div className="hero-card-container fade-up mobile-no-3d" style={{ animationDelay: "0.3s", display: "flex", flexDirection: "column", alignItems: "center", gap: 40, transformStyle: "preserve-3d", transform: `translateZ(50px) rotateY(${mousePos.x * -10}deg) rotateX(${mousePos.y * 10}deg)`, transition: "transform 0.2s ease-out" }}>
            <div style={{ perspective: "1500px", position: "relative", width: "100%", maxWidth: 420, aspectRatio: "1.58" }}>
              ...
              <div className="shadow-p" style={{ position: "absolute", bottom: -40, left: "10%", width: "80%", height: 30, background: "radial-gradient(ellipse, rgba(0,0,0,0.8) 0%, transparent 70%)", filter: "blur(15px)" }}/>
            </div>
            <div style={{ display: "flex", gap: 0, background: "rgba(31, 41, 55, 0.6)", backdropFilter: "blur(20px)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 20, overflow: "hidden", transform: "translateZ(30px)", boxShadow: "0 20px 40px rgba(0,0,0,0.4)" }}>
              {[{ num: "2.4K+", label: "Pros" },{ num: "1 Tap", label: "Connexion" },{ num: "100%", label: "Écolo" }].map((s, i) => (
                <div key={i} style={{ padding: "16px 24px", textAlign: "center", borderRight: i < 2 ? "1px solid rgba(255,255,255,0.05)" : "none" }}>
                  <div style={{ fontFamily: "var(--font-display)", fontSize: "clamp(16px, 4vw, 22px)", fontWeight: 800, color: "var(--text-main)" }}>{s.num}</div>
                  <div style={{ fontFamily: "var(--font-body)", fontSize: 11, color: "var(--brand-primary)", letterSpacing: "0.1em", textTransform: "uppercase", marginTop: 4, fontWeight: "700" }}>{s.label}</div>
                </div>
              ))}
            </div>
          </div>
...
// ─── FEATURES ────────────────────────────────────────────────────────────────
const featuresInfo = [
  { title: 'Technologie NFC', description: 'Un simple contact suffit pour partager votre profil. Pas de scan, pas de friction, juste une connexion instantanée.', icon: <Nfc className="w-8 h-8 text-[#F5A623]" />, color: 'from-[#F5A623]/20 to-[#D97706]/20', borderColor: 'group-hover:border-[#F5A623]/50' },
  { title: 'Analyses en Temps Réel', description: 'Suivez qui consulte votre carte et quand. Transformez vos rencontres en opportunités mesurables.', icon: <BarChart className="w-8 h-8 text-[#8B5CF6]" />, color: 'from-[#8B5CF6]/20 to-[#6D28D9]/20', borderColor: 'group-hover:border-[#8B5CF6]/50' },
  { title: 'Personnalisation Totale', description: 'Choisissez parmi nos designs premium ou créez le vôtre. Votre carte doit être aussi unique que votre marque.', icon: <Palette className="w-8 h-8 text-[#06B6D4]" />, color: 'from-[#06B6D4]/20 to-[#0891B2]/20', borderColor: 'group-hover:border-[#06B6D4]/50' },
];

function Features() {
  return (
    <section id="features" className="py-24 px-4 sm:px-6 lg:px-8 bg-[#0B0F19] relative overflow-hidden border-t border-white/5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {featuresInfo.map((feature, index) => (
            <motion.div key={index} initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7, delay: index * 0.2 }} whileHover={{ y: -10, scale: 1.02 }} className="relative group">
              <div className={`h-full bg-[#111827] p-10 rounded-3xl border border-white/5 ${feature.borderColor} transition-all duration-500 relative overflow-hidden flex flex-col items-start shadow-xl`}>
                <div className={`absolute inset-0 bg-gradient-to-br ${feature.color} opacity-0 group-hover:opacity-100 transition-opacity duration-500`} />
                <div className="relative mb-8 p-4 rounded-2xl bg-white/5 border border-white/10 group-hover:scale-110 group-hover:rotate-6 transition-transform duration-500 shadow-xl">
                  {feature.icon}
...
function HowItWorksCombined() {
  ...
  return (
    <section id="showcase" className="py-24 px-4 sm:px-6 lg:px-8 bg-[#111827] relative overflow-hidden border-t border-white/5">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
              ...
              {combinedSteps.map((step, index) => (
                <motion.div key={index} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.2 }} viewport={{ once: true }} className="flex gap-6 group">
                  <div className="flex flex-col items-center">
                    ...
                    {index !== combinedSteps.length - 1 && <div className="w-px h-full bg-gradient-to-b from-white/20 to-transparent my-2" />}
                  </div>
                  <div className="pb-6">
                    <div className="text-sm font-bold text-[#F5A623] mb-1">Étape {step.number}</div>
          ...
          <div className="relative flex justify-center items-center h-[600px] w-full pt-20 overflow-visible">
            <motion.div animate={{ y: scanState === 'scanning' ? 80 : -50, scale: scanState === 'scanning' ? 0.9 : 1, rotateX: scanState === 'success' ? 45 : 15, opacity: scanState === 'success' ? 0 : 1 }} transition={{ duration: 0.6, ease: "easeInOut" }} style={{ perspective: "1000px" }} className="absolute top-0 z-50 flex justify-center w-full">
              {/* Petite Carte Animée (Dark Theme) */}
              <div className="w-40 h-28 rounded-xl bg-[#111827] border border-white/20 shadow-[0_20px_40px_rgba(0,0,0,0.8)] flex items-center justify-center relative overflow-hidden transform rotate-12">
                <div className="absolute inset-0 bg-gradient-to-tr from-[#F5A623]/10 to-transparent" />
                <CreditCard className="text-gray-400 w-6 h-6 absolute top-2 left-2" />
                <span className="text-[#F5A623] font-bold text-lg tracking-widest z-10" style={{ fontFamily: 'var(--font-logo)', textShadow: '1.5px -1.5px 0px #06B6D4, -1.5px 1.5px 0px #8B5CF6' }}>DIMA</span>
                <Nfc className="text-[#F5A623] w-5 h-5 absolute bottom-2 right-2" />
              </div>
            </motion.div>
            
            <div className="relative z-10 w-[280px] h-[560px] bg-black rounded-[2.5rem] border-[6px] border-[#1F2937] shadow-[0_0_60px_rgba(0,0,0,0.8)] overflow-hidden">
              <div className="absolute top-2 left-1/2 -translate-x-1/2 w-24 h-6 bg-[#1F2937] rounded-full z-50" />
              <div className="relative w-full h-full bg-[#0a0a0a]">
                <AnimatePresence mode="wait">
                  ...
                  {scanState === 'success' && (
                    <motion.div key="success" initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} transition={{ type: "spring", damping: 25, stiffness: 200 }} className="absolute inset-0 bg-[#0B0F19] overflow-y-auto pb-6 scrollbar-hide">
                      ...
                      <div className="px-5 -mt-10 relative z-10 flex flex-col items-center">
                        <div className="w-20 h-20 rounded-full border-4 border-[#0B0F19] bg-[#111827] flex items-center justify-center shadow-xl"><span className="text-2xl font-bold text-white" style={{ fontFamily: 'var(--font-display)' }}>MM</span></div>
                        <div className="mt-3 text-center">
                          <h2 className="text-xl font-bold text-white" style={{ fontFamily: 'var(--font-display)' }}>Mohammed Modroussi</h2>
                          <p className="text-[#F5A623] font-medium text-xs mt-1 uppercase tracking-wider">CEO DimaCard</p>
                          <p className="text-gray-500 text-[10px] mt-2 flex items-center justify-center gap-1"><MapPin className="w-3 h-3" /> Tanger, Maroc</p>
                        </div>
                        
                        <button className="w-full mt-5 bg-[#F5A623] text-black font-bold py-3 rounded-xl flex items-center justify-center gap-2 text-sm hover:bg-[#D97706] transition-colors border-none cursor-pointer" style={{ fontFamily: 'var(--font-body)' }}>
                          <UserPlus className="w-4 h-4" /> Enregistrer le contact
                        </button>

                        <div className="w-full mt-5 grid grid-cols-2 gap-3">
                          {[
                            { icon: <Phone className="w-5 h-5 text-[#F5A623] mb-2" />, text: "+212 70 80 66 509" },
                            { icon: <Mail className="w-5 h-5 text-[#8B5CF6] mb-2" />, text: "contact@dimacard.ma" },
                            { icon: <Linkedin className="w-5 h-5 text-[#06B6D4] mb-2" />, text: "/in/m-modroussi" },
                            { icon: <Globe className="w-5 h-5 text-[#F5A623] mb-2" />, text: "www.dimacard.ma" }
                          ].map((item, idx) => (
                            <div key={idx} className="flex flex-col items-center justify-center p-3 bg-white/5 border border-white/10 rounded-xl hover:bg-white/10 transition-colors cursor-pointer text-center">
                              {item.icon}
                              <span className="text-[9px] text-gray-400 font-medium truncate w-full">{item.text}</span>
                            </div>
                          ))}
                        </div>
                      </div>
...
const plans = [
  { 
    ...
    features: ['1 Carte PVC Noir Mat', 'Profil digital de base', 'Idéal pour les événements'], 
    highlight: false, 
    productId: 'etudiant' 
  },
  { 
  ...
  { 
    ...
    features: ['1 Carte Premium Or', 'Profil 100% modifiable', 'Accès complet plateforme'], 
    highlight: true, 
    productId: 'pro' 
  },
  { 
...
function Pricing({ setCurrentPage }) {
  return (
    <section id="pricing" className="py-24 px-4 sm:px-6 lg:px-8 bg-[#0B0F19] relative overflow-hidden border-t border-white/5">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 items-stretch">
          {plans.map((plan, index) => (
            <motion.div key={index} initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} whileHover={{ y: -10 }} transition={{ duration: 0.5, delay: index * 0.1 }} viewport={{ once: true }} className={`relative p-6 xl:p-8 rounded-3xl transition-all duration-500 flex flex-col h-full ${plan.highlight ? 'bg-[#1F2937] border-2 border-[#F5A623] shadow-[0_20px_50px_rgba(245,166,35,0.15)] md:scale-105 z-20' : 'bg-[#111827] border border-white/10 hover:border-white/20 shadow-xl z-10'}`}>
              {plan.highlight && (
                <div className="absolute -top-4 left-1/2 -translate-x-1/2 bg-[#F5A623] text-black text-[10px] font-bold px-4 py-1.5 rounded-full uppercase tracking-widest flex items-center gap-1 whitespace-nowrap shadow-lg shadow-[#F5A623]/30">
              ...
              <div className="mb-8">
                ...
                <div className="flex items-baseline gap-1">
                  ...
                  {plan.price !== 'Sur devis' && <span className="text-gray-500 text-xs uppercase font-bold tracking-wider" style={{ fontFamily: 'var(--font-body)' }}>/une fois</span>}
                </div>
              </div>
              ...
              <button onClick={() => setCurrentPage(`product_${plan.productId}`)} className={`w-full py-4 px-2 rounded-xl font-bold transition-all duration-300 cursor-pointer border-none text-sm xl:text-base ${plan.highlight ? 'bg-[#F5A623] text-black hover:bg-[#D97706] shadow-lg shadow-[#F5A623]/20' : 'bg-white/10 text-white hover:bg-white/20'}`} style={{ fontFamily: 'var(--font-body)' }}>
...
function Contact() {
  ...
  return (
    <section id="contact" className="py-24 px-4 sm:px-6 lg:px-8 bg-[#111827] relative overflow-hidden border-t border-white/5">
      <div className="absolute top-0 left-1/4 w-96 h-96 bg-[#F5A623]/5 blur-[100px] rounded-full pointer-events-none" />
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-12 bg-[#1F2937] border border-white/10 rounded-3xl p-6 sm:p-10 shadow-2xl relative z-20">
          <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 0.8, delay: 0.2 }} viewport={{ once: true }} className="lg:col-span-2 space-y-8">
            ...
            <div className="space-y-6">
              <div className="flex items-start gap-4"><div className="w-12 h-12 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center shrink-0"><Mail className="w-5 h-5 text-[#F5A623]" /></div><div><h4 className="text-white font-bold mb-1">Email</h4><a href="mailto:contact@dimacard.ma" className="text-gray-400 hover:text-[#F5A623] transition-colors text-sm font-medium">contact@dimacard.ma</a></div></div>
              <div className="flex items-start gap-4"><div className="w-12 h-12 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center shrink-0"><Phone className="w-5 h-5 text-[#F5A623]" /></div><div><h4 className="text-white font-bold mb-1">Téléphone</h4><a href="tel:+212708066509" className="text-gray-400 hover:text-[#F5A623] transition-colors text-sm font-medium">+212 7 08 06 65 09</a></div></div>
              <div className="flex items-start gap-4"><div className="w-12 h-12 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center shrink-0"><MapPin className="w-5 h-5 text-[#F5A623]" /></div><div><h4 className="text-white font-bold mb-1"> Bureau</h4><p className="text-gray-400 text-sm font-medium">Tanger, Maroc</p></div></div>
            </div>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 0.8, delay: 0.4 }} viewport={{ once: true }} className="lg:col-span-3">
            <form className="space-y-6" onSubmit={handleSubmit}>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div className="space-y-2"><label htmlFor="name" className="text-sm font-bold text-gray-300">Nom complet</label><input type="text" id="name" required value={formData.name} onChange={handleChange} className="w-full bg-[#111827] border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-[#F5A623] focus:ring-1 focus:ring-[#F5A623] transition-colors" /></div>
                <div className="space-y-2"><label htmlFor="email" className="text-sm font-bold text-gray-300">Adresse Email</label><input type="email" id="email" required value={formData.email} onChange={handleChange} className="w-full bg-[#111827] border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-[#F5A623] focus:ring-1 focus:ring-[#F5A623] transition-colors" /></div>
              </div>
              <div className="space-y-2"><label htmlFor="subject" className="text-sm font-bold text-gray-300">Sujet</label><input type="text" id="subject" required value={formData.subject} onChange={handleChange} className="w-full bg-[#111827] border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-[#F5A623] focus:ring-1 focus:ring-[#F5A623] transition-colors" /></div>
              <div className="space-y-2"><label htmlFor="message" className="text-sm font-bold text-gray-300">Message</label><textarea id="message" required rows="4" value={formData.message} onChange={handleChange} className="w-full bg-[#111827] border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-[#F5A623] focus:ring-1 focus:ring-[#F5A623] transition-colors resize-none"></textarea></div>
              ...
              <button type="submit" disabled={isSubmitting || isSuccess} className={`w-full sm:w-auto px-8 py-4 font-bold rounded-xl flex items-center justify-center gap-2 border-none cursor-pointer text-black shadow-lg transition-all ${isSuccess ? 'bg-[#10B981] text-white shadow-[#10B981]/20' : 'bg-[#F5A623] hover:bg-[#D97706] shadow-[#F5A623]/30 hover:shadow-[#F5A623]/50'} disabled:opacity-70 disabled:cursor-not-allowed`} style={{ fontFamily: 'var(--font-body)' }}>
      ...
      <div style={{ maxWidth: 1280, margin: "0 auto", padding: "0 24px" }}>
        <div className="footer-grid" style={{ display: "grid", marginBottom: 64 }}>
          <div className="flex flex-col items-center md:items-start">
            <div style={{ marginBottom: 20 }}><span className="brand-logo-text" style={{ fontSize: 28, textShadow: "-1.5px 2px 0px var(--brand-secondary)" }}>DIMA CARD</span></div>
            <p style={{ fontFamily: "var(--font-body)", fontSize: 15, color: "var(--text-muted)", lineHeight: 1.7, marginBottom: 24, maxWidth: 300 }}>La carte de visite NFC la plus intelligente, conçue pour les professionnels.</p>
            <div className="footer-socials" style={{ display: "flex", gap: 12 }}>
              {["𝕏","in","ig"].map((s, i) => <a key={i} href="#" style={{ width: 36, height: 36, borderRadius: "50%", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14, color: "var(--text-main)", textDecoration: "none", fontWeight: "600", transition: "background 0.3s" }} onMouseEnter={e=>e.currentTarget.style.background="rgba(255,255,255,0.1)"} onMouseLeave={e=>e.currentTarget.style.background="rgba(255,255,255,0.05)"}>{s}</a>)}
            </div>
          </div>
          {[ { title: "Produit", links: ["Fonctionnalités","Tarifs","Comment ça marche"] }, { title: "Entreprise", links: ["À propos","Contact"] }, { title: "Légal", links: ["Confidentialité","CGV"] } ].map((col, i) => (
            <div key={i}>
              ...
              <ul style={{ listStyle: "none", display: "flex", flexDirection: "column", gap: 12 }}>
                {col.links.map(l => <li key={l}><a href="#" style={{ fontFamily: "var(--font-body)", fontSize: 15, color: "var(--text-muted)", textDecoration: "none", fontWeight: "500", transition: "color 0.2s" }} onMouseEnter={e=>e.currentTarget.style.color="var(--brand-primary)"} onMouseLeave={e=>e.currentTarget.style.color="var(--text-muted)"}>{l}</a></li>)}
              </ul>
            </div>
          ))}
        </div>
        <div style={{ borderTop: "1px solid var(--border-light)", paddingTop: 32, display: "flex", justifyContent: "space-between", flexWrap: "wrap", gap: 16 }}>
          ...
          <p style={{ fontFamily: "var(--font-body)", fontSize: 14, color: "var(--text-muted)", fontWeight: "500" }}>Conçu avec 💛 au Maroc</p>
        </div>
      </div>
    </footer>
  );
    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="pt-32 pb-20 px-6 max-w-7xl mx-auto min-h-screen">
      ...
      <div className="flex gap-4 mb-12 overflow-x-auto pb-4 scrollbar-hide">
        {Object.values(productsData).map(p => (
          <button 
            ...
            className={`px-6 py-3 rounded-full font-bold whitespace-nowrap transition-all border-2 cursor-pointer ${product.id === p.id ? 'bg-[#F5A623] text-black border-[#F5A623] shadow-lg shadow-[#F5A623]/30' : 'bg-[#1F2937] text-gray-300 border-white/10 hover:border-white/30'}`}
          >
            {p.name}
          </button>
        ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
        ...
        <div className="space-y-8">
          ...
          <div className="relative rounded-3xl overflow-hidden flex items-center justify-center p-8 sm:p-16 shadow-2xl border border-white/10 bg-[#111827]" style={{ minHeight: '400px' }}>
          ...
          <div>
            <div className="inline-block px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-widest mb-4 border text-[#F5A623] bg-[#F5A623]/10 border-[#F5A623]/30 shadow-sm">
              {product.tag}
            </div>
          ...
          <div className="space-y-4">
            {product.features.map((feat, i) => (
              <div key={i} className="flex items-center gap-4 p-4 bg-[#1F2937] border border-white/10 rounded-2xl shadow-sm">
        ...
        <div className="bg-[#1F2937] border border-white/10 rounded-3xl p-8 shadow-2xl relative overflow-hidden">
          ...
          <div className="flex justify-between items-end mb-8 border-b border-white/10 pb-6 relative z-10">
            <div>
              <h2 className="text-2xl font-black text-white" style={{ fontFamily: 'var(--font-display)' }}>Configurer la carte</h2>
              <p className="text-sm text-gray-400 mt-1">Saisissez les infos à intégrer sur la puce.</p>
            </div>
            <span className="text-3xl font-black text-[#F5A623]" style={{ fontFamily: 'var(--font-display)' }}>{product.price}</span>
          </div>

          <form onSubmit={handleOrderSubmit} className="space-y-5 relative z-10">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              <div className="space-y-2">
                ...
                <input type="text" required value={orderData.fullName} onChange={e => setOrderData({...orderData, fullName: e.target.value})} className="w-full bg-[#111827] border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-[#F5A623] focus:ring-1 transition-colors" placeholder="Ex: Jean Dupont" />
              </div>
              <div className="space-y-2">
                ...
                <input type="text" required value={orderData.jobTitle} onChange={e => setOrderData({...orderData, jobTitle: e.target.value})} className="w-full bg-[#111827] border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-[#F5A623] focus:ring-1 transition-colors" placeholder="Ex: CEO & Fondateur" />
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <input type="text" value={orderData.company} onChange={e => setOrderData({...orderData, company: e.target.value})} className="w-full bg-[#111827] border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-[#F5A623] focus:ring-1 transition-colors" placeholder="Ex: DimaTech S.A.R.L" />
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              <div className="space-y-2">
                ...
                <input type="tel" required value={orderData.phone} onChange={e => setOrderData({...orderData, phone: e.target.value})} className="w-full bg-[#111827] border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-[#F5A623] focus:ring-1 transition-colors" placeholder="+212 6 XX XX XX XX" />
              </div>
              <div className="space-y-2">
                ...
                <input type="email" required value={orderData.email} onChange={e => setOrderData({...orderData, email: e.target.value})} className="w-full bg-[#111827] border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-[#F5A623] focus:ring-1 transition-colors" placeholder="contact@entreprise.ma" />
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <input type="url" value={orderData.linkedin} onChange={e => setOrderData({...orderData, linkedin: e.target.value})} className="w-full bg-[#111827] border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-[#F5A623] focus:ring-1 transition-colors" placeholder="https://linkedin.com/in/votreprofil" />
            </div>

            {errorMsg && (
              <div className="flex items-center gap-2 text-red-400 text-sm bg-red-400/10 p-3 rounded-lg border border-red-400/20 mt-4">
                <AlertCircle className="w-4 h-4" /> {errorMsg}
              </div>
            )}

            <button type="submit" disabled={isSubmitting || isSuccess} className={`w-full py-5 mt-6 font-black rounded-xl flex items-center justify-center gap-2 transition-all cursor-pointer text-black border-none shadow-xl ${isSuccess ? 'bg-[#10B981] text-white shadow-[#10B981]/30' : 'bg-[#F5A623] hover:bg-[#D97706] shadow-[#F5A623]/30'} disabled:opacity-80`} style={{ fontFamily: 'var(--font-body)' }}>
            ...
            <p className="text-center text-xs text-gray-500 mt-3 font-medium">Paiement sécurisé à la livraison. Expédition 48h au Maroc.</p>
          </form>
        </div>
      </div>
    </motion.div>
